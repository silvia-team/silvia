## code to prepare `aldo_Ref_Prod_Bois_stocks` dataset goes here
...
aldo_Ref_Prod_Bois_stocks <- france_harvested_wood %>%
  group_by(EPCI_Siren) %>%
  summarise(wood_carbon_stocks = round(sum(carbon_stocks), digits=1))

aldo_Ref_Prod_Bois_stocks <- as.data.table(aldo_Ref_Prod_Bois_stocks)


usethis::use_data(aldo_Ref_Prod_Bois_stocks, overwrite = TRUE)
