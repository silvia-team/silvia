...
download_impermability_layers <- function(shape, years = c("12", "15")){
  ...
  for (year in years){
    ...
    impermab <- brick(file_path, package= "raster")
    impermab <- aggregate(impermab, fact=6)
    impermab <- mean(impermab)
    raster::writeRaster(impermab, filename = file_path, overwrite= T)
